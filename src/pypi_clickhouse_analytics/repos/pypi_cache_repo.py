...
class PypiCacheRepo:
    ...
    async def get_project_meta(self, project_name: str) -> ProjectMeta | None:
        project_meta: dict | None = await self.__cache.get(self.__project_meta_cache_key(project_name))
        if project_meta is not None:
            return ProjectMeta.model_validate(project_meta)

    async def set_project_meta(self, project_name: str, project_meta: ProjectMeta):
